...
そこで、その考えを Rust でゼロから作り直しました。超高速な tmux 互換マルチプレクサ、型付き SDK、永続セッション、構造化スナップショット、そして Linux、macOS、Windows のネイティブなローカルトランスポートを備えています。Windows Named Pipes も含み、WSL は不要です。
